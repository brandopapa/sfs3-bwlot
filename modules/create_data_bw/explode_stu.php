...
if ($do_key =="Excel 輸出") {	
	...
	while (!$result->EOF) {
		...
		//}
		$query2 = "select fath_name,fath_phone,fath_home_phone,fath_hand_phone,moth_name,moth_phone,moth_home_phone,moth_hand_phone,guardian_name,guardian_phone from stud_domicile where stud_id ='$stud_id'";
    ...
    $guardian_phone = $result2->fields[guardian_phone];
		...
		echo sprintf("<td>%d</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td>",substr($curr_class_num,1,2),substr($curr_class_num,-2),$stud_birthday,$stud_person_id,$school,$fath_name,$fath_phone,$fath_home_phone,$fath_hand_phone,$moth_name,$moth_phone,$moth_home_phone,$moth_hand_phone,$guardian_name,$guardian_phone,$addr_zip); 

		//echo sprintf("<td>=T(\"%s\")</td><td>%s</td><td>=T(\"%s\")</td>",$s_home_phone,stripslashes($s_addres),$s_offical_phone); 
    echo sprintf("<td>=T(\"%s\")</td><td>%s</td><td>%s</td><td>=T(\"%s\")</td>",$s_home_phone,$addr_1,$addr_2,$s_offical_phone); 
