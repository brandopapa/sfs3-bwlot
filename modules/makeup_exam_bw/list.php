...
if ($_POST['cal'] || $_POST['export'] || $_POST['insert'] || $_POST['noti1']  || $_POST['list']) {
	...
	$chk_lang = $m_arr[$_POST['subj_lang']]['e']<>"";
	$chk_math = $m_arr[$_POST['subj_math']]['e']<>"";
	$chk_nature = $m_arr[$_POST['subj_natu']]['e']<>"";
	$chk_social = $m_arr[$_POST['subj_soci']]['e']<>"";
	$chk_health = $m_arr[$_POST['subj_heal']]['e']<>"";
	$chk_art = $m_arr[$_POST['subj_art']]['e']<>"";
	$chk_complex = $m_arr[$_POST['subj_comp']]['e']<>"";
	$chk_any = ($chk_lang || $chk_math || $chk_nature || $chk_social || $chk_health || $chk_art || $chk_complex);
	
	//if ($_POST['export'] && $m_arr[$_POST['subj']]['e']<>"") {
	if ($_POST['export'] && $chk_any) {
			//$filename=$sel_year."-".$sel_seme."-".$_POST['class_year']."年級-補測".$m_arr[$_POST['subj']]['c'].".csv";
			$filename=$sel_year."-".$sel_seme."-".$_POST['class_year']."年級-補測";
			if ($chk_lang){
				$filename=$filename.$m_arr[$_POST['subj_lang']]['c'];
				$smarty->assign("ename_lang",$m_arr[$_POST['subj_lang']]['e']);
				$smarty->assign("cname_lang",$m_arr[$_POST['subj_lang']]['c']);				
			}
			if ($chk_math){
				$filename=$filename.$m_arr[$_POST['subj_math']]['c'];
				$smarty->assign("ename_math",$m_arr[$_POST['subj_math']]['e']);
				$smarty->assign("cname_math",$m_arr[$_POST['subj_math']]['c']);				
			}
			if ($chk_nature){
				$filename=$filename.$m_arr[$_POST['subj_natu']]['c'];
				$smarty->assign("ename_nature",$m_arr[$_POST['subj_natu']]['e']);
				$smarty->assign("cname_nature",$m_arr[$_POST['subj_natu']]['c']);				
			}
			if ($chk_social){
				$filename=$filename.$m_arr[$_POST['subj_soci']]['c'];
				$smarty->assign("ename_social",$m_arr[$_POST['subj_soci']]['e']);
				$smarty->assign("cname_social",$m_arr[$_POST['subj_soci']]['c']);								
			}
			if ($chk_health){
				$filename=$filename.$m_arr[$_POST['subj_heal']]['c'];
				$smarty->assign("ename_health",$m_arr[$_POST['subj_heal']]['e']);
				$smarty->assign("cname_health",$m_arr[$_POST['subj_heal']]['c']);								
			}
			if ($chk_art){
				$filename=$filename.$m_arr[$_POST['subj_art']]['c'];
				$smarty->assign("ename_art",$m_arr[$_POST['subj_art']]['e']);
				$smarty->assign("cname_art",$m_arr[$_POST['subj_art']]['c']);												
			}
			if ($chk_complex){
				$filename=$filename.$m_arr[$_POST['subj_comp']]['c'];
				$smarty->assign("ename_complex",$m_arr[$_POST['subj_comp']]['e']);
				$smarty->assign("cname_complex",$m_arr[$_POST['subj_comp']]['c']);												
			}
