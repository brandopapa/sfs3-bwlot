...
<tr align="right">
<td colspan="30" align="center" style="font-size: 16pt;font-family: 標楷體;font-weight: bold;">{{$sch.sch_cname}}學生平時定考成績證明書<br><br>
<table border="0">
...
<tr align="center">
<td style="border-style: solid; border-width: 1.5pt;" rowspan="3"><font style="font-size: 8pt;">科目\學期</font></td>
{{foreach from=$all_score item=d key=i}}
{{foreach from=$d item=dd key=j}}
<td colspan="6" style="border-style: solid; border-width: 1.5pt 1.5pt 0.75pt 0pt;"><font style="font-size: 8pt;">{{$i}}學年度<br>第{{$j}}學期</font></td>
{{/foreach}}
{{/foreach}}
</tr>
...
<td colspan="2" style="border-style: solid; border-width: 0pt {{if $t==3}}1.5{{else}}0.75{{/if}}pt 1.5pt 0pt;"><font style="font-size: 8pt;">{{$t}}</font></td>
{{/foreach}}
{{/foreach}}
{{/foreach}}
</tr>
...
{{foreach from=$all_score item=d key=i}}
{{foreach from=$d item=dd key=j}}
{{foreach from=$test_sort item=t key=k}}
{{foreach from=$test_kind item=tk}}
<td style="border-style: solid; border-width: 0pt {{if $t==3}}1.5{{else}}0.75{{/if}}pt 1.5pt 0pt;"><font style="font-size: 8pt;">{{$tk}}</font></td>
{{/foreach}}
{{/foreach}}
{{/foreach}}
{{/foreach}}
...
{{foreach from=$test_sort item=t key=n}}
{{foreach from=$test_kind item=tk}}
<td style="border-style:solid; border-width:0pt {{if $t==3}}1.5{{else}}0.75{{/if}}pt 0.75pt 0pt;">{{if $dddd.$i.$t.$tk == ""}}---{{else}}{{$dddd.$i.$t.$tk}}{{/if}}</td>
{{/foreach}}
{{/foreach}}
{{/foreach}}
{{/foreach}}
