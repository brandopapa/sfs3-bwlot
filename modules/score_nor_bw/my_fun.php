...
class data_student {
	...
	function get_seme_data($student_sn="") {
		...
		$cnum_arr=array("1"=>"一","2"=>"二","3"=>"三","4"=>"四","5"=>"五","6"=>"六","7"=>"七","8"=>"八","9"=>"九","10"=>"總計");
	...
	function get_abs_data($stud_id="",$group_id) {
		...
		if (!empty($stud_id)) {
			...
			while(!$res->EOF) {
				...
				$this->abs_total[$res->fields['abs_kind']]+=$res->fields['abs_days'];
				$res->MoveNext();
			}
		}
	}
	...
	function get_rew_data($stud_id="",$group_id) {
		...
		if (!empty($stud_id)) {
			...
			while(!$res->EOF) {
				...
				$this->rew_total[$res->fields['sr_kind_id']]+=$res->fields['sr_num'];
				$res->MoveNext();
			}
		}
	}
