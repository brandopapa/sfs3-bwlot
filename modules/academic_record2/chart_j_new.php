...
//努力程度
$oth_arr_score = array("值得讚賞"=>5,"表現良好"=>4,"表現尚可"=>3,"需再加油"=>2,"有待改進"=>1);
$oth_arr_score_2 = array(5=>"值得讚賞",4=>"表現良好",3=>"表現尚可",2=>"需再加油",1=>"有待改進");
...
function &main_form($interface_sn="",$sel_year="",$sel_seme="",$class_id="",$student_sn=""){
	...
	$score_data = &get_score_value_bw($stud_id,$student_sn,$class_id,$oth_data);	

	//取得詳細資料
	//$html=&html2code2($class,$sel_year,$sel_seme,$oth_data,$nor_data,$abs_data,$reward_data,$score_data,$student_sn,($chk_menu_arr)?1:0);
	$html=&html2code2forbw($class,$sel_year,$sel_seme,$oth_data,$nor_data,$abs_data,$reward_data,$score_data,$student_sn,($chk_menu_arr)?1:0);
	...
	$c_curr_seme = sprintf("%03d",curr_year()).curr_seme();
	...
	$grid1->sql_str = "select a.student_sn,a.stud_name,a.stud_sex,substring(a.curr_class_num,4,2)as sit_num  from stud_base a,stud_seme b where a.stud_study_cond = 0 and a.student_sn=b.student_sn and b.seme_year_seme='$c_curr_seme' and b.seme_class='$class[2]' order by a.curr_class_num";
	$grid1->do_query(); //執行命令 
	...
	<td>學號：<font color='purple'>$stu[stud_id]</font></td>
