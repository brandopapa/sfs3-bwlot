...
$grade_group_id=$_REQUEST[grade_group_id];
...
if($act=="儲存登記"){
	...
	$main=&mainForm($sel_year,$sel_seme,$class_id,$stud_id,$grade_group_id);
}else{
	$main=&mainForm($sel_year,$sel_seme,$class_id,$grade_group_id);
}
...
function &mainForm($sel_year,$sel_seme,$class_id="",$stud_id="",$grade_group_id=""){	
	...
	if(!empty($class_id) and !empty($stud_id)){
		...
		$signForm=&stud_statForm($sel_year,$sel_seme,"$year-$month-$day",$class_id,$stud_id,$grade_group_id);
  ...
  $grade_group_array['grade_group_2']="非領域科目缺席記錄";
	...
	//日曆的連結字串
	$linkStr=(!empty($class_id) and !empty($stud_id))?"&start_date=$start_year-$start_month-$start_day&act=view_one&grade_group_id=$grade_group_id&class_id=$class_id&stud_id=$stud_id":"&start_date=$start_year-$start_month-$start_day&class_id=$class_id&grade_group_id=$grade_group_id";
	...
	if(!empty($class_id)){
		...
		<tr bgcolor='#FEFBDA'>
		<td align='center'>		
		<a href='$_SERVER[PHP_SELF]?act=$_REQUEST[act]&this_day=$today&class_id=$class_id&grade_group_id=$grade_group_id' class='box'><img src='".$SFS_PATH_HTML."images/today.png' alt='回到今天' width='16' height='16' hspace='2' border='0' align='absmiddle'>回到今天</a>
		</td></tr>
	...
	$class_select $stu_sel $grade_group_select
...
function &statForm($sel_year,$sel_seme,$class_id,$grade_group_id){
	...
	foreach($stud as $id=>$name){
		...
		$aaa=getOneMdata($id,$sel_year,$sel_seme,"$year-$month-$day",$class_id,$grade_group_id,"",$start_date);
		//各一節資料
		$blank="";
		for($i=1;$i<=$all_sections;$i++){
			$blank.="<td>$aaa[$i]</td>";
		...
		<td>$id</td>
		<td>".$num[$id]."</td>
		<td><a href='$_SERVER[PHP_SELF]?act=view_one&class_id=$class_id&grade_group_id=$grade_group_id&stud_id=$id&this_date=$year-$month-$day&start_date=$start_date'>$name</a></td>		
		$uf
		$sections_data
		$df
		$all_day
...
function &stud_statForm($sel_year,$sel_seme,$this_date,$class_id,$stud_id,$grade_group_id){
	...
	if ($grade_group_id != "grade_group_1" && $grade_group_id != "grade_group_2"){//全部
	  $sql="select date,absent_kind,section from stud_absent where (date>='$start_date') and (date<='$this_date') and stud_id='$stud_id' order by date,section";
		...
		//else{
			// $sql_condition ="";
	  //}			
		$sql="select date,absent_kind,section from stud_absent where (date>='$start_date') and (date<='$this_date') and stud_id='$stud_id' ".$sql_condition." order by date,section";
	}
