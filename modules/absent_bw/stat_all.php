...
function &mainForm($sel_year,$sel_seme,$class_id="",$grade_group_id=""){
  ...
  $grade_group_array['grade_group_2']="非領域科目缺席記錄";
	...
	<a href='$_SERVER[PHP_SELF]?act=print&class_id=$class_id'>列印</a>
...
function &statForm($sel_year,$sel_seme,$class_id,$grade_group_id,$mode){
	...
	if ($mode=="print") {
		$print_str="	<html><head><title>$s[sch_cname]".$sel_year."學年度第".$sel_seme."學期".$c[3]."年".$c[4]."班缺曠課統計表</title></head>\n
			<body><p align='center'><font face='標楷體' size='4'>$s[sch_cname]".$sel_year."學年度第".$sel_seme."學期</font><br><font face='標楷體' size='5'>".$c[3]."年".$c[4]."班缺曠課統計表</font></p>\n
