...
$main=&mainForm($sel_year,$sel_seme,$week_num,$grade_group_id);
...
//主要輸入畫面
function &mainForm($sel_year,$sel_seme,$week_num="",$grade_group_id){
	global $school_menu_p,$year,$month,$day,$SFS_PATH_HTML,$CONN,$today,$start_day,$weeks_array;
	//相關功能表
	$tool_bar=&make_menu($school_menu_p);
	
	//週報表
	if ($week_num) $weekForm=wform($sel_year,$sel_seme,$week_num,$grade_group_id);
...
//	$linkStr=(!empty($week_num))?"&week_num=$week_num":"";
	...
	$week_select $grade_group_select
	<input type='hidden' name='act' value='view'>
	<input type='hidden' name='this_date' value='$year-$month-$day'>
	勤惰週報表<input type='submit' name='act' value='分班列印'><input type='submit' name='act' value='合併列印'><input type='submit' name='act' value='列印通知書'></td></form></tr>
	</table>
...
function wform($sel_year,$sel_seme,$week_num,$grade_group_id) {
	global $CONN,$weekN,$class_year,$start_day,$weeks_array;
	...
	$sql="select a.*,b.seme_num,c.stud_name from stud_absent a, stud_seme b, stud_base c where a.date >= '$start_day[st_start]' and a.date <= '$wd[6]' and a.stud_id=b.stud_id and b.student_sn=c.student_sn and b.seme_year_seme='$seme_year_seme' order by a.class_id,b.seme_num,a.date,a.section";
	$rs=$CONN->Execute($sql);
	if ($rs->recordcount() > 0) {
		$m=0;
		while (!$rs->EOF) {
