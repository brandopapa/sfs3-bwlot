		...
		*/
		...
		if($single[2] && $stud_id) {
			...
			if($student_sn){
				...
				$CONN->Execute("UPDATE stud_base SET stud_person_id='{$single[7]}',stud_birthday='$birthday',stud_tel_2='{$single[23]}',stud_tel_3='{$single[24]}',addr_zip='{$single[25]}',stud_addr_2='{$single[26]}' WHERE student_sn={$student_sn}") or user_error("讀取失敗！<br>$sql",256);
				//更新監護人姓名
				$CONN->Execute("UPDATE stud_domicile SET guardian_name='{$single[22]}' WHERE student_sn={$student_sn}") or user_error("讀取失敗！<br>$sql",256);
			} echo "<br>班級：{$seme_class} 學號：{$stud_id} 姓名：{$single[6]} 因為找不到本學期就學紀錄，資料無法更新！";
			/*
			echo "<pre>";
			print_r($single);
			echo "</pre>";
...
echo "<br>※更新回寫判斷依據：學號、班級、座號三者須相符";
echo "<br>※會更新回寫的欄位：身分證號、出生年、出生月、出生日、家長姓名、市內電話、行動電話、郵遞區號、通訊地址";
