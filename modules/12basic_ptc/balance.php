...
if($work_year==$academic_year and $editable_sn_array) $tool_icon.=" <input type='submit' value='統計本年度所有開列學生均衡學習的級分' name='act' onclick='return confirm(\"確定要重新統計本年度所有開列學生均衡學習的級分?\")'>
		<input type='submit' value='清除所有開列學生均衡學習的級分' name='act' onclick='return confirm(\"確定要\"+this.value+\"?\")'>";
...
if($stud_class)
{
	...
	$recordSet=$CONN->Execute($stud_select) or user_error("讀取失敗！<br>$stud_select",256);
	$studentdata="<tr align='center' bgcolor='#ff8888'><td width=80>學號</td><td width=50>座號</td><td width=120>姓名</td><td width=$pic_width>大頭照</td><td width=50>健體</td><td width=50>藝文</td><td width=50>綜合</td><td>級分統計</td><td>備註</td>";
	while(list($student_sn,$seme_num,$stud_name,$stud_sex,$stud_id,$stud_study_year)=$recordSet->FetchRow()) {
		...
		if($student_sn==$edit_sn){
			...
			//動作按鈕
			$action="<input type='submit' name='act' value='確定修改' onclick='return confirm(\"確定要修改 $stud_name 的均衡學習級分資料?\")'> <input type='submit' name='act' value='取消' onclick='document.myform.edit_sn.value=0;'>";		
