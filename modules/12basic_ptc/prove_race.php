...
//顯示對照
$level_arr=array("1"=>"國際性","2"=>"全國性","3"=>"全縣性");
...
while(!$rs->EOF){
	...
	if($res->RecordCount()) {
		...
		<tr align='center'><td>NO.</td><td>學年度</td><td>競賽類別</td><td>範圍</td><td>競賽名稱</td><td>得獎名次</td><td>證書日期</td><td>主辦單位</td><td>字號</td><td>採計</td></tr>";  //<td>性質</td>
		$no=0;
		while(!$res->EOF)
		{
			$level=$level_arr[$res->fields['level']];
			$weight=$res->fields['weight']?'是':'';
			if($level) {				
				$no++;
				echo "<tr align='center'><td>$no</td><td>{$res->fields['year']}</td><td>{$res->fields['nature']}</td><td>$level</td><td align='left'>{$res->fields['name']}</td><td>{$res->fields['rank']}</td><td>{$res->fields['certificate_date']}</td><td align='left'>{$res->fields['sponsor']}</td><td align='left'>{$res->fields['word']}</td><td>$weight</td></tr>"; //<td>{$res->fields['squad']}</td>
			}
			$res->MoveNext();
		}
		echo "</table></div>";		
