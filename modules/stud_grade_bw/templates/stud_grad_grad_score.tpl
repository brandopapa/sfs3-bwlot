...
{{if $sl=="chinese"}}<td rowspan="{{$ss_num}}">{{if $fin_score.$sn.avg.score < 60}}<font color="red">{{/if}}{{$fin_score.$sn.avg.score}}<br>({{$fin_score.$sn.avg.str}}){{if $fin_score.$sn.avg.score < 60}}</font>{{/if}}<br><BR><BR>({{$fin_score.$sn.total.score}})</td>{{/if}}
