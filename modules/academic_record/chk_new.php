...
$w_arr=array("====","值得讚賞","表現良好","表現尚可","須再加油","有待改進");		
...
//if($class_num) {
	...
	$memo_nor=$res->fields['ss_score_memo'];//平常文字描述
  $rowdata['nor']['memo']=$memo_nor;
	...
	$nor_val[$i]="";
	while (!$res->EOF) {
		$i=$res->fields['ss_id'];//平常項次
		$ssval=$res->fields['ss_val'];//平常努力程度
	...
	$rowdata['nor']['ss_item']=array(1=>"平常行為表現",2=>"團體活動表現",3=>"公共服務",4=>"校外特殊表現");
...
$smarty->assign("module_name","填寫學生平常生活表現檢核表");
