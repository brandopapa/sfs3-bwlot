<?php
// $Id: make.php 6280 2011-01-07 05:58:24Z infodaes $

// 系統選項
$performance=array(1=>"平常行為表現",2=>"團體活動表現",3=>"公共服務",4=>"校外特殊表現");
$performance_option=array(1=>"值得讚賞",2=>"表現良好",3=>"表現尚可",4=>"需再加油",5=>"有待改進");
...
include_once "../../include2/sfs_case_dataarray.php";
include_once "../../include2/sfs_oo_zip2.php";
sfs_check();
...
function &main_form($sel_year,$sel_seme,$class_id,$stud_id){
	...
	if(!empty($class_id)){
		...
		if(!empty($stud_id)){
			...
			$stud_all.=make_list($nor_text,"平常生活表現文字","","",false);
			
			//取得學生平常生活檢核文字
			$chk_text=get_chk_text($student_sn,$sel_year,$sel_seme,$CHK_KIND);
			$stud_all.=make_list($chk_text,"平常生活檢核文字","","",false);
