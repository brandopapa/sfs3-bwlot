...
// $Id: write_memo.php 7322 2013-06-27 11:41:44Z smallduh $
include "config.php";
include_once "../../include2/sfs_case_PLlib.php";
include_once "../../include2/sfs_case_score.php";
...
if($_POST[act]=="存檔"){
	...
	while(list($id,$val) = each($sss_id_arr)){
		if ($val<>''){
			...
			//$query = "update stud_seme_score set ss_score_memo = '$_POST[$temp_val]',teacher_sn='$_SEESION[session_tea_sn]' where sss_id='$val'";
			$query = "update stud_seme_score set ss_score_memo = '$_POST[$temp_val]' where sss_id='$val'";
	...
	if ($_POST['thepaste']) {
	  ...
	  foreach($data_arr as $a) {
	   ...
	   if ($stud_sss_id) {
			...
			$query = "update stud_seme_score set ss_score_memo = '$score_memo' where sss_id='$stud_sss_id'";
	    mysql_query($query);
	   } // end if $stud_sss_id
...
if(strstr ($teacher_course, 'g')){
	...
	$sql="select a.student_sn from stud_base a, stud_seme b  where a.curr_class_num like '$class_id%' and a.stud_study_cond='0' and a.student_sn = b.student_sn and b.seme_year_seme = '$seme_year_seme' and b.seme_class='$class_id'";  
}
...
if ($res->fields[0]<$stud_numbers) {
	...
	while (!$res->EOF){
		...
		if (empty($rs->fields[student_sn])) {
			...
			$query = "INSERT INTO stud_seme_score(seme_year_seme,student_sn,ss_id)values('$seme_year_seme','$sst','$ss_id')";
			$CONN->Execute($query);
		}
		$res->MoveNext();
	}
...
// 取得努力程度文字敘述
//$arr_1 = sfs_text("努力程度");
$arr_1 = array("1"=>"值得讚賞","2"=>"表現良好","3"=>"表現尚可","4"=>"需再加油","5"=>"有待改進");
// 取得學生努力程度值
...
$main="
	...
	remote = window.open(\"../academic_record2/comment.php?cq=\"+p,\"MyNew\", strFeatures);
	if (remote != null) {
...
$(document).ready(function(){
	if ($("select[name='teacher_course']").val())
		$("#showPrevMemo").attr('disabled','');
	else
		$("#showPrevMemo").attr('disabled','disabled');
