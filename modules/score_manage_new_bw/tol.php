<?php
// $Id: tol.php 6542 2011-09-23 03:07:07Z infodaes $
/*引入設定檔*/
include "config.php";
...
$score_area_id = $_REQUEST['score_area'];
$go=$_POST['go'];
...
//選擇四大主軸 add by david 20150515
if($me) $score_area_menu=my_score_area_menu($score_area_id);	
//if($me) {
if($score_area_id) {
...
$menu="<form name=\"myform\" method=\"post\" action=\"$_SERVER[PHP_SELF]\">
	<table>
	<tr>
	<td>$year_seme_menu</td><td>$class_year_menu</td><td>$class_year_name_menu</td><td>$score_area_menu$percision_radio</td>
	</tr>
	</table></form>";
...
if($year_name && $me && $percision){
	...
	$ss_val=array("值得讚賞"=>"5","表現良好"=>"4","表現尚可"=>"3","需再加油"=>"2","有待改進"=>"1");
	...
	if ($score_area_id == '' || $score_area_id =='all'){
		...
		if ($score_area_id =='領域主軸'){		
			$area_condition= "and link_ss in ('語文-本國語文','語文-鄉土語文','語文-英語','數學','自然與生活科技','社會','健康與體育','藝術與人文','綜合活動')";	
		}
		else{
			$area_condition= "and link_ss ='$score_area_id'";	
		}			
	}		
	$sql="select * from score_ss where class_id='".sprintf("%03s_%s_%02s_%02s",$sel_year,$sel_seme,$year_name,$me)."' and enable='1' $area_condition";
	$rs=$CONN->Execute($sql);
	if ($rs->RecordCount() ==0){
		$sql="select * from score_ss where year='$sel_year' and semester='$sel_seme' and class_year='$year_name' and enable='1' and need_exam='1' and class_id='' $area_condition order by sort,sub_sort";
		$rs=$CONN->Execute($sql);
	}
	...
	while (!$rs->EOF) {
		...
		if ($friendly_print==1) {
			...
			//$subject_rate_list.="<td width='40' align='center' colspan='2'><p align='left'><span style='font-size:10pt;'>".$s_rate[$id]."</span></p></td>";
		} 
		else if($save_csv ==1){
			$subject_kind.=stripslashes($subject_name[$ss_id[$id]])."分數,".stripslashes($subject_name[$ss_id[$id]])."努力程度,";			
			$subject_rate_list.=$s_rate[$id].",,";
		}	else if($save_csv ==2){
    ...
    header("Content-type: text/x-csv ; Charset=Big5");
		...
		$print_msg="<a href='{$_SERVER['PHP_SELF']}?year_seme=$year_seme&year_name=$year_name&me=$me&friendly_print=1&percision=$_REQUEST[percision]&score_area=$_REQUEST[score_area]' target='new'><b><small>友善列印</small></b></a>&nbsp;&nbsp;&nbsp;"; 
		$print_msg.="<a href='{$_SERVER['PHP_SELF']}?year_seme=$year_seme&year_name=$year_name&me=$me&save_csv=1&percision=$_REQUEST[percision]&score_area=$_REQUEST[score_area]'><b><small>匯出本班的csv檔</small></b></a>&nbsp;&nbsp;&nbsp;";
		$print_msg.="<a href='{$_SERVER['PHP_SELF']}?year_seme=$year_seme&year_name=$year_name&me=$me&save_csv=2&percision=$_REQUEST[percision]&score_area=$_REQUEST[score_area]'><b><small>匯出全年級csv檔(不含努力程度)</small></b></a><br>";
	...
	$help_text="努力程度： 5:值得讚賞, 4:表現良好, 3:表現尚可, 2:需再加油, 1:有待改進";
	echo $main;
