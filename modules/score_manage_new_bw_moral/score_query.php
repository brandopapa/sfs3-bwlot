...
$kind=$_REQUEST['kind'];
$subject_scope=$_REQUEST['subject_scope'];
...
if($year_seme){
	...
	$show_subject_scope = array(79=>'基礎主軸',77=>'生命主軸');
	$ss2 = new drop_select();
	$ss2->s_name ="subject_scope";
	$ss2->top_option = "選擇主軸";
	$ss2->id = $subject_scope;
	$ss2->arr = $show_subject_scope;
	$ss2->is_submit = true;
	$subject_scope_menu =$ss2->get_select();
...
$menu="<form name=\"myform\" method=\"post\" action=\"$_SERVER[SCRIPT_NAME]\">
	<table>
	<tr>
	<td>$year_seme_menu</td><td>$class_year_menu</td><td>$stage_menu</td><td>$kind_menu</td><td>$subject_scope_menu</td>
	</tr>
	</table>
	<table>
...
//----end of 取得排除列表 student_sn (98.12.20)
	...
	if ($subject_scope == 77 || $subject_scope == 79){
		$query="select distinct b.ss_id from $score_semester a,score_ss b where b.class_year='$c_year' and  a.test_sort='$stage' and a.ss_id=b.ss_id and a.student_sn in ($all_sn) and b.enable='1' and b.scope_id = $subject_scope order by b.sort,b.sub_sort";
	}
	else{
	  $query="select distinct b.ss_id from $score_semester a,score_ss b where b.class_year='$c_year' and  a.test_sort='$stage' and a.ss_id=b.ss_id and a.student_sn in ($all_sn) and b.enable='1' and b.scope_id in (77,79) order by b.sort,b.sub_sort";
  }
	//echo $query."<br>";
	...
	while (list($k,$v)=each($choice_kind)) {
		...
		$sql="select a.*,b.rate from $score_semester a,score_ss b where a.test_sort='$stage' and a.test_kind='$v' and a.ss_id=b.ss_id and a.student_sn in ($all_sn) and b.enable='1' and b.scope_id = 82 order by b.sort,b.sub_sort";
		if ($subject_scope == 77 || $subject_scope == 79){
			$sql="select a.*,b.rate from $score_semester a,score_ss b where a.test_sort='$stage' and a.test_kind='$v' and a.ss_id=b.ss_id and a.student_sn in ($all_sn) and b.enable='1' and b.scope_id = $subject_scope order by b.sort,b.sub_sort";
	  }
		else{
			$sql="select a.*,b.rate from $score_semester a,score_ss b where a.test_sort='$stage' and a.test_kind='$v' and a.ss_id=b.ss_id and a.student_sn in ($all_sn) and b.enable='1' and b.scope_id in(77,79) order by b.sort,b.sub_sort";
		}
		//echo $sql."<br>";
		$rs=$CONN->Execute($sql) or die($query);
		$ossid="";
