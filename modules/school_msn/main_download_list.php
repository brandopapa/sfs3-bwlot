...
<title>æª?æ¡?ä¸?è¼?</title>
</head>
<body>
<table border="0" width="100%">
	<tr>
		<td align="left" style="font-size:10pt" style="color:#FF0000">Â§è«??¸æ??æª?æ¡?é¡???</td>
...
<table border="1" cellpadding="0" cellspacing="0" width="100%" bordercolorlight="#800000" bordercolordark="#FFFFFF" bordercolor="#FFFFFF">
    <tr>
      <td width="60" bgcolor="#FFFFCC" style="font-size:10pt" align="center">?¥æ??</td>
      <td width="60" bgcolor="#FFCCCC" style="font-size:10pt" align="center">ä¸??³è??</td>
      <td width="240" bgcolor="#CCFFCC" style="font-size:10pt" align="center">æª?æ¡?èªªæ??</td>
      <td bgcolor="#FFCCCC" style="font-size:10pt" align="center">æª???</td>
    </tr>
<?php
...
$result=mysql_query($query);
*/
$query="select id,idnumber,teach_id,post_date,data from sc_msn_data where to_id='' and data_kind=2 and folder=? order by post_date desc";
...
$stmt->execute();
$stmt->bind_result($id,$idnumber,$teach_id,$post_date,$data);
///mysqli


//while ($row=mysql_fetch_row($result)) {
  while ($stmt->fetch()) {
	$row = array($id,$idnumber,$teach_id,$post_date,$data);
 ...
 if (mysql_num_rows($result_file)) {
  ...
  ?>
    ...
    <?php echo $filename_r;?>&nbsp;
  	<a href="main_download.php?set=<?php echo $filename;?>">ä¸?è¼?<?php echo $j;?></a>
  	(<?php echo $file_download;?>æ¬?)
  	<br>
     <?php
        } // end while file
     ?>  
