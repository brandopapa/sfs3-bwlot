...
{
	global $CONN,$fault_none,$fault_warning,$fault_peccadillo,$reward_score;
	$fault=array();
	$sn_list=implode(',',$sn_array);
	//抓取學生未銷過的獎懲紀錄
	$sql="SELECT student_sn,reward_year_seme,reward_kind FROM reward WHERE student_sn IN ($sn_list) AND reward_cancel_date='0000-00-00'";  //10/28 16:00 與技專校院招生策進總會確認：銷過>的紀錄當作沒發生
	$res=$CONN->Execute($sql) or user_error("讀取失敗！<br>$sql",256);
	while(!$res->EOF)
	{
		$student_sn=$res->fields['student_sn'];
