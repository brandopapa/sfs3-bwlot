...
// $Id: book_input.php 7728 2013-10-28 09:02:05Z smallduh $
...
while ($row = mysql_fetch_array($result) ){
	...
	if ($qbook_name !=""){
		...
		echo sprintf("<tr %s><td>%3d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><a href=\"%s?book_id=%s&bookch1_id=%s&page=%d&qbook_name=%s&showpage=%s\">修改</a></td></tr>\n",$bgcolor,$page*$showpage+$stopi+1,$row["book_id"],str_replace($qbook_name,$replace_text,$row["book_name"]),str_replace($qbook_name,$replace_text,$row["book_author"]),str_replace($qbook_name,$replace_text,$row["book_maker"]),"book_edit.php",$row["book_id"],$bookch1_id,$page,$qbook_name,$showpage);
	}
	else{
		echo sprintf("<tr %s><td>%3d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><a href=\"%s?book_id=%s&bookch1_id=%s&page=%d&qbook_name=%s&showpage=%s\">修改</a></td></tr>\n",$bgcolor,$page*$showpage+$stopi+1,$row["book_id"],$row["book_name"],$row["book_author"],$row["book_maker"],"book_edit.php",$row["book_id"],$bookch1_id,$page,$qbook_name,$showpage);
	}
	$ci++;
	$stopi++;
}
