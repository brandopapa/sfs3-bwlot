...
// $Id: qbookout_tea_list.php 6803 2012-06-22 07:56:42Z smallduh $
...
echo  "</tr>";
while($row = mysql_fetch_array($result)){
	$query2 ="select name from teacher_base where teach_id ='".$row["stud_id"]."'";
	$result2 = mysql_query($query2) or die ($query2);
	$row2 = mysql_fetch_array($result2);
	$cyear = $row2["curr_class_num"];
	$out_d = $row["out_d"] ;
	if ($row["yet"] > $tea_yetdate)
		$out_d = "<font color=red>".$row["out_d"]."</font>" ;
	echo sprintf("<tr><td>%s</td><td>%s</td><td nowrap>%s</td><td nowrap>%s</td></tr>",$row["book_id"],$row["book_name"],$row2["name"],$out_d);
}
echo "</table>";
echo "</center>";
include "footer.php";
