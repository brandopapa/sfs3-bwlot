...
if($selected_stud && $_POST['act']=='輸出成績證明單') {
	...
	foreach($selected_stud as $student_key=>$student_sn) {
		...
		//表一：學生基本資料
		$stud_name=str_replace(' ','',$student_data[$student_sn]['stud_name'].'('.$student_data[$student_sn]['stud_id'].')');		//姓名
		...
		$data.="<div style='margin:20px 0;'>";
		$data.="<span style='font-size:18pt;'>二、品德服務：</span><br><span style='font-size:15pt;'>1.獎勵紀錄：大功\3分、小功\1分、嘉獎0.3分，最高10分。<br>2.出缺席紀錄：無曠課者5分、曠課1至5節者3分、曠課6至10節者1分。<br>3.無記過紀錄：無警告以上紀錄者(含銷過後)5分、銷過後無累積至小過(含)以上者1分。</span>";
		...
		//$data.="<tr align='center'><th colspan='2'>合計</th><td>".($reward_data[$grade71][9]+$reward_data[$grade72][9]+$reward_data[$grade81][9]+$reward_data[$grade82][9]+$reward_data[$grade91][9])."</td><td>".($reward_data[$grade71][3]+$reward_data[$grade72][3]+$reward_data[$grade81][3]+$reward_data[$grade82][3]+$reward_data[$grade91][3])."</td><td>".($reward_data[$grade71][1]+$reward_data[$grade72][1]+$reward_data[$grade81][1]+$reward_data[$grade82][1]+$reward_data[$grade91][1])."</td><td>".($absence_data[$grade81]+$absence_data[$grade82]+$absence_data[$grade91])."</td><td>".($reward_data[$grade81][-1]+$reward_data[$grade82][-1]+$reward_data[$grade91][-1]-$reward_data['fault_cancel'][-1])."</td><td>".($reward_data[$grade81][-3]+$reward_data[$grade82][-3]+$reward_data[$grade91][-3]-$reward_data['fault_cancel'][-3])."</td><td>".($reward_data[$grade81][-9]+$reward_data[$grade82][-9]+$reward_data[$grade91][-9]-$reward_data['fault_cancel'][-9])."</td></tr>";
  	$data.="<tr align='center'><th colspan='2'>合計</th><td>".($reward_data[$grade71][9]+$reward_data[$grade72][9]+$reward_data[$grade81][9]+$reward_data[$grade82][9]+$reward_data[$grade91][9])."</td><td>".($reward_data[$grade71][3]+$reward_data[$grade72][3]+$reward_data[$grade81][3]+$reward_data[$grade82][3]+$reward_data[$grade91][3])."</td><td>".($reward_data[$grade71][1]+$reward_data[$grade72][1]+$reward_data[$grade81][1]+$reward_data[$grade82][1]+$reward_data[$grade91][1])."</td><td>".($absence_data[$grade71]+$absence_data[$grade72]+$absence_data[$grade81]+$absence_data[$grade82]+$absence_data[$grade91])."</td><td>".($reward_data[$grade71][-1]+$reward_data[$grade72][-1]+$reward_data[$grade81][-1]+$reward_data[$grade82][-1]+$reward_data[$grade91][-1]-$reward_data['fault_cancel'][-1])."</td><td>".($reward_data[$grade71][-3]+$reward_data[$grade72][-3]+$reward_data[$grade81][-3]+$reward_data[$grade82][-3]+$reward_data[$grade91][-3]-$reward_data['fault_cancel'][-3])."</td><td>".($reward_data[$grade71][-9]+$reward_data[$grade72][-9]+$reward_data[$grade81][-9]+$reward_data[$grade82][-9]+$reward_data[$grade91][-9]-$reward_data['fault_cancel'][-9])."</td></tr>";
		...
		//表三：多元學習
		$data.="<div style='margin:20px 0;'>";
		$data.="<span style='font-size:18pt;'>三、多元學習：</span><br><span style='font-size:15pt;'>1.圴衡學習：藝術與人文、健康與體育、綜合活動學期總平均成績及格者，每一領域給3分。<br>2.體適能：金質章7分、銀質章6分、銅質章5分、2-4項達25%給4分、1項達25%給2分。<br>3.競賽表現：最高7分。(需檢附獎狀影印本或參賽證明)</span>";
		$data.="<table border='2' cellpadding='3' cellspacing='0' style='width:100%; font-size:14pt; border-collapse:collapse;' bordercolor='#111111'>";
		...
		$data.="</table><br>";
		$data.="<table border='2' cellpadding='3' cellspacing='0' style='width:100%; font-size:14pt; border-collapse:collapse;' bordercolor='#111111'>";
		$data.="<tr align='center'><th>NO</th><th>範圍</th><th>性質</th><th>競賽名稱</th><th>證書日期</th><th>名次</th><th>得分</th><th>採記</th></tr>";
		for($i=1; $i<=count($competetion); $i++) {
			...
			$data.="<tr align='center'><td>{$i}</td><td>{$level_array[$competetion[$i]['level']]}</td><td>{$squad_array[$competetion[$i]['squad']]}{$team}</td><td>{$competetion[$i]['name']}</td><td>{$competetion[$i]['certificate_date']}</td><td>{$competetion[$i]['rank']}</td><td>{$competetion[$i]['score']}</td><td>{$competetion[$i]['mark']}</td></tr>";
		}
		$data.="<tr align='center'><th colspan='6'>合計</th><td>{$final_data[$student_sn]['score_competetion']}</td><td bgcolor='#D6D3D6'></td></tr>";
		$data.="</table>";
		$data.="</div>";
		//四：簽名、核章
