<?php
// $Id: show.php 8650 2015-12-18 03:58:09Z qfon $
include "config.php";
...
if ($stud_id <> "") {
    ...
    $stmt = $mysqliconn->prepare("select a.id,b.title,a.order_pos,a.data_get,a.data_input,a.up_date,a.class_id,b.doc,b.grada from cita_data a inner join cita_kind b on a.kind=b.id where (a.stud_id = ?  and a.order_pos>-1) order by $der  ");
    $stmt->bind_param('s', $stud_id);

} 
$stmt->execute();
$stmt->bind_result($did, $item, $order_pos, $data_get, $data_input, $up_date, $class_id, $doc, $gra);
