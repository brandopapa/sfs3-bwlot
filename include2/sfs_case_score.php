<?php
// $Id: sfs_case_score.php 7422 2013-08-15 08:00:35Z infodaes $
...
// 取得成績檔
function &get_score_value_bw($stud_id,$student_sn,$class_id,$oth_data,$sel_year,$sel_seme,$stage="") {
	global $CONN,$oth_arr_score,$oth_arr_score_2;
	$class=class_id_2_old($class_id);
	// 取得努力程度文字敘述
//	$arr_1 = sfs_text("努力程度");
...
$ss_score_avg['平均']['平時成績']=round($ss_score_sum['平時成績'],0);
	...
	if ($stage=="") {
		$temp_str.="
			<td align=\"center\">節數</td>
			<td align=\"center\">等第(分數)</td>
			<td align=\"center\">努力程度(學習能力)</td>
			</tr>";
		$ss_sql_select = "select link_ss,count(ss_id) as cc from score_ss where enable='1' and class_id='$class_id' and need_exam='1' group by link_ss order by sort,sub_sort";
		$ss_recordSet=$CONN->Execute($ss_sql_select) or user_error("讀取失敗！<br>$ss_sql_select",256);
		if ($ss_recordSet->RecordCount() ==0){
	...
	$hidden_str=($hidden_ss_id)?"<input type=\"hidden\" name=\"hidden_ss_id\" value=\"$hidden_ss_id\">\n":"";
	return $temp_str.$hidden_str."</table>";
}
...
// $score_data -- 成績記錄
// $mode 0:不含檢核表, 1:含檢核表
function &html2code2forbw($class,$sel_year,$sel_seme,$oth_data,$nor_data,$abs_data,$reward_data,$score_data,$student_sn,$mode=0) {
	global $SFS_PATH_HTML,$TOTAL_DAYS,$CONN,$IS_JHORES,$is_summary_input;
	$arr_1 = sfs_text("品德言行表現");
	$arr_2 = sfs_text("團體活動表現");
	$arr_3 = sfs_text("公共服務");
	$arr_4 = sfs_text("校外特殊表現");
	//假別
	...
	if ($mode==1) {
		$temp_str.= "
		<tr bgcolor=\"#c4d9ff\">
		<td colspan=\"13\" align=\"center\" nowrap>學習領域評量</td></tr>";
	} else {
		$temp_str.= "
		<tr bgcolor=\"white\">
		<td nowrap>其他</td></tr>";
		...
		if  ($IS_JHORES==0&&$_SESSION[session_who]=="教師")
			$temp_str.= "<td colspan=\"12\"><input type='text' name='oth_rep' id='oth_rep' value='".$oth_data['其他設定'][0]."'></td></tr>";
		else
			$temp_str.= "<td colspan=\"12\">".$oth_data['其他設定'][0]."</td></tr>";
	}
		
	$temp_str .="</table></td></tr><tr><td>
	$score_data	
	</td></tr>
	<tr>
	<td>
	...
	if ($mode==1) {
		$temp_str .="
		<tr bgcolor=\"#c4d9ff\">
		<td colspan=\"13\" align=\"center\" nowrap>平常生活表現評量</td>
		</tr>
		<tr bgcolor=\"#c4d9ff\">
		<td align=\"center\" nowrap>評量項目</td>
		<td colspan=\"4\" align=\"center\" nowrap>努力程度</td>
		<td colspan=\"8\" align=\"center\" nowrap>導師評語及建議</td>
		</tr>
		<tr bgcolor=\"white\">
		<td nowrap>品德言行表現</td><td colspan=\"4\" align=\"center\">".$oth_data['生活表現評量'][1]."</td>
		<td rowspan=\"4\" colspan=\"8\"><textarea name='nor_score_memo' id='nor_score_memo' rows=7 style='height:100%;width: 100%'>".$nor_data[ss_score_memo][0]."</textarea></td>
		</tr>
		<tr bgcolor=\"white\">
		<td nowrap>團體活動表現</td><td colspan=\"4\" align=\"center\">".$oth_data['生活表現評量'][2]."</td>
		</tr>
		<tr bgcolor=\"white\">
		<td nowrap>公共服務</td><td colspan=\"4\" align=\"center\">".$oth_data['生活表現評量'][3]."</td>
		</tr>
		<tr bgcolor=\"white\">
		<td nowrap>校外特殊表現</td><td colspan=\"4\" align=\"center\">".$oth_data['生活表現評量'][4]."</td>
		</tr>
		";
	} else {
		$temp_str .="
...
if($is_summary_input=='y' || $IS_JHORES!=0) {
	...
	$temp_str.="
	<td nowrap>出缺席記錄，<br/><a href=\"absent.php\">填寫勤惰記錄</a> $summary_input<br>
	</td>";
	while(list($id,$val)=each($abs_kind_arr)){
		$ttt =($IS_JHORES==0)?"天數":"節數";
		if ($id==4) $ttt= "次數";
