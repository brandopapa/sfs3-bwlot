...
function &get_ss_name_arr($class,$mode="長"){
	...
	if ($res->RecordCount() ==0){
		...
		//$query = "select s.subject_name,ss.* from score_ss ss left join score_subject s on s.subject_id = ss.subject_id where ss.year='$class[0]' and ss.semester='$class[1]' and ss.class_year='$class[3]' and ss.class_id='' and ss.enable='1' order by ss.sort,ss.sub_sort";
		$res = $CONN->Execute($query);
	}
	while (!$res->EOF) {
    ...
    //print $ss_id."-".$scope_name."-".$subject_name."_".$show_ss.";";    	
